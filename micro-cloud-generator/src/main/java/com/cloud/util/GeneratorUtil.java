...
import com.cloud.modules.system.model.GeneratorModel;

import java.util.ArrayList;
import java.util.List;

/**
 * 代码生成器工具类 参考MyBatis Plus 官网文档
 */
public class GeneratorUtil {

    public static void  generateCode(GeneratorModel model){
        // 生成代码路径
        String projectPath = System.getProperty("user.dir") + "/" + model.getServicesName();
        // 模块名
        String modelName = model.getModelName();
        String[] tableNames = model.getTableName().split(",");
        // 代码生成器
        AutoGenerator autoGenerator = new AutoGenerator();
        // 设置模板引擎(freemarker模板引擎)
        // Velocity模板引擎有提示,但是语法我不会
        // autoGenerator.setTemplateEngine(new VelocityTemplateEngine());
        autoGenerator.setTemplateEngine(new FreemarkerTemplateEngine());
        // 初始化全局配置
        autoGenerator.setGlobalConfig(initGlobalConfig(projectPath,model.getAuthor()));
        // 初始化数据源配置
        autoGenerator.setDataSource(initDataSourceConfig());
        // 初始化包配置
        autoGenerator.setPackageInfo(initPackageConfig(modelName));
        // 初始化模板配置
        autoGenerator.setTemplate(initTemplateConfig());
        // 初始化策略配置
        autoGenerator.setStrategy(initStrategyConfig(tableNames));
        // 初始化自定义配置
        autoGenerator.setCfg(initInjectionConfig(projectPath, modelName));
        // 执行代码生成器
     ...
     * 初始化全局配置
     */
    private static GlobalConfig initGlobalConfig(String projectPath,String author) {
    ...
    private static TemplateConfig initTemplateConfig() {
        ...
        // 对controller、service、entity模板进行配置
